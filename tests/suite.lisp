...
(defun dehaze-node-validates-and-encodes-p ()
  "A dehaze node carries an amount, refuses one off its range, fills its
default in, and reaches the executor as kind 20."
  (flet ((rejected-p (params &optional (input-kind :crop))
           (handler-case
               (progn
                 (graph-validate
    ...
    (let* ((params '(:amount 0.6))
      ...
      (and (equal params (graph-node-params (graph-find-node decoded 1)))
           (= 20 (elt bytes 12))
           (= 1 (elt bytes 24))
           (member :dehaze (graph-node-kinds))
           (equal '(:amount 0.3) (dehaze-default-params))
           (not (rejected-p (dehaze-default-params)))
           (not (rejected-p '()))
           (rejected-p '(:amount 2))
           (rejected-p '(:amount -1.5))
           (rejected-p '(:radius 3))
  ...
  (let* ((directory (asdf:system-relative-pathname "orfeus" #P"data/luts/"))
         (expected
           '(("IWLTBAP K25.cube" . "4065c4c9cdbe1df5e37f43538710ae340e877879ccf31875f91e0702d2bc03c4")
             ...
             ("JSZ-CEDAR A.cube" . "1c9caa016e56f5bc5614d7b08e3b0e32dfba1b712b27952b636753532a5db511")
             ("JSZ-CEDAR B.cube" . "3da92e353dad4585ecc0a7bae78ce6a4af66520692d6f8cde37a61daff7d533b")
             ("JSZ-CEDAR C.cube" . "57e4acc0162e41e1fc1cfc4f2b05882cf8ca188e281049db67234b369d696414")
             ("JSZ-CEDAR D.cube" . "39fa2235cb42d3d0ce2cbab277a1469716d9b6cdee349c59b691803918b90d75")
             ("JSZ-CEDAR E.cube" . "e37da2a569886fff5da70b354c8f326df3394f3e905c4336dd27bd3d6cd9ec5e")
             ("JSZ-CEDAR F.cube" . "c6aec89110ad4134be84c541bf496c5b846dcf7b77fcbfb3e9860018fcdcca1d")
             ("JSZ-CEDAR G.cube" . "907f77111dba605d5b8ab9003f266db765039e8784282963b9b7843644879f73")
             ("JSZ-CEDAR H.cube" . "60c7891f23e4c26bda8fed1a66490f7ae1b737d673e3edef82612949b30457f5")
...
(defun run-tests ()
  ...
  (let ((failures 0))
    (flet ((check (description predicate)
      ...
      (check "dehaze node validates, encodes and defaults"
             (dehaze-node-validates-and-encodes-p))
