...
selection the way every desktop does, and the stash keeps each frame once."
  ;; Names with numbers read by value, not by character.
  (check (orfeus/gui::natural-string< "P8294003.ORF" "P8294010.ORF")
         "Frame 3 did not come before frame 10")
  (check (orfeus/gui::natural-string< "P9.ORF" "P10.ORF")
         "Nine did not come before ten")
  (check (not (orfeus/gui::natural-string< "b" "A"))
         "Case decided the order")
  ;; A directory lists folders and the files the filter admits, hidden names
  ;; left out, in the order asked for.
  (let ((directory (uiop:ensure-directory-pathname
                    (format nil "/tmp/orfeus-picker-test-~D/" (get-universal-time)))))
    (ensure-directories-exist (merge-pathnames "sub/" directory))
    (ensure-directories-exist (merge-pathnames ".hidden/" directory))
    (dolist (name '("P8294010.ORF" "P8294003.orf" "notes.txt" ".DS_Store" "scan.dng" "snap.JPG"))
      (with-open-file (stream (merge-pathnames name directory) :direction :output
                                                                :if-exists :supersede)
        (write-string name stream)))
    (multiple-value-bind (folders files)
        (orfeus/gui::picker-list-directory directory :filter :raw :sort :name)
      (check (equal '("sub") (mapcar #'orfeus/gui::picker-entry-name folders))
             "The hidden folder was listed or the plain one was not")
      (check (equal '("P8294003.orf" "P8294010.ORF" "scan.dng")
                    (mapcar #'orfeus/gui::picker-entry-name files))
             "RAW filter listed ~S" (mapcar #'orfeus/gui::picker-entry-name files))
      (check (every (lambda (entry) (plusp (orfeus/gui::picker-entry-size entry))) files)
             "A file was listed without its size"))
    (multiple-value-bind (folders files)
        (orfeus/gui::picker-list-directory directory :filter :pictures :sort :name)
      (declare (ignore folders))
      (check (equal '("P8294003.orf" "P8294010.ORF" "scan.dng" "snap.JPG")
                    (mapcar #'orfeus/gui::picker-entry-name files))
             "Pictures filter listed ~S" (mapcar #'orfeus/gui::picker-entry-name files)))
    (multiple-value-bind (folders files)
        (orfeus/gui::picker-list-directory directory :filter :all :sort :name)
      (declare (ignore folders))
      (check (= 5 (length files)) "Everything did not list the five plain files"))
    ;; The thumbnail cache key follows the file's identity.
    (let* ((entry (first (nth-value 1 (orfeus/gui::picker-list-directory directory))))
           (same (orfeus/gui::picker-thumbnail-pathname entry #P"/cache/"))
           (touched (orfeus/gui::make-picker-entry
                     (orfeus/gui::picker-entry-name entry)
                     (orfeus/gui::picker-entry-pathname entry)
                     :size (1+ (orfeus/gui::picker-entry-size entry))
                     :mtime (orfeus/gui::picker-entry-mtime entry))))
      (check (equal same (orfeus/gui::picker-thumbnail-pathname entry #P"/cache/"))
             "The cache key wandered")
      (check (not (equal same (orfeus/gui::picker-thumbnail-pathname touched #P"/cache/")))
             "A changed file kept its old thumbnail"))
    (uiop:delete-directory-tree directory :validate t))
  ;; Clicks: plain, Control, Shift, and the keyboard.
  (let ((selection (orfeus/gui::make-picker-selection)))
    (orfeus/gui::picker-selection-click selection 2)
    (check (equal '(2) (orfeus/gui::picker-selection-list selection)) "A click did not select")
    (orfeus/gui::picker-selection-click selection 5 :shift t)
    (check (equal '(2 3 4 5) (orfeus/gui::picker-selection-list selection))
           "Shift-click did not select the range")
    (orfeus/gui::picker-selection-click selection 3 :shift t)
    (check (equal '(2 3) (orfeus/gui::picker-selection-list selection))
           "A second shift-click did not resize the range from the anchor")
    (orfeus/gui::picker-selection-click selection 7 :control t)
    (check (equal '(2 3 7) (orfeus/gui::picker-selection-list selection))
           "Control-click did not add")
    (orfeus/gui::picker-selection-click selection 3 :control t)
    (check (equal '(2 7) (orfeus/gui::picker-selection-list selection))
           "Control-click did not take out")
    (check (= 3 (orfeus/gui::picker-selection-cursor selection))
           "The cursor did not follow the click")
    (orfeus/gui::picker-selection-move selection 4)
    (check (equal '(4) (orfeus/gui::picker-selection-list selection))
           "An arrow did not move the selection with the cursor")
    (orfeus/gui::picker-selection-move selection 6 :shift t)
    (check (equal '(4 5 6) (orfeus/gui::picker-selection-list selection))
           "Shift-arrow did not extend the range")
    (orfeus/gui::picker-selection-toggle-cursor selection)
    (check (equal '(4 5) (orfeus/gui::picker-selection-list selection))
           "Space did not take the cursor's frame out")
    (orfeus/gui::picker-selection-all selection 8)
    (check (equal '(0 1 2 3 4 5 6 7) (orfeus/gui::picker-selection-list selection))
           "Select all missed some")
    (orfeus/gui::picker-selection-band selection '(1 2) '(6 7) :control t)
    (check (equal '(1 2 6 7) (orfeus/gui::picker-selection-list selection))
           "A band with Control did not add to what was there")
    (orfeus/gui::picker-selection-band selection '(3) '(6 7))
    (check (equal '(3) (orfeus/gui::picker-selection-list selection))
           "A plain band did not replace the selection")
    ;; Removing renumbers what is left and lands the cursor sensibly.
    (orfeus/gui::picker-selection-all selection 6)
    (orfeus/gui::picker-selection-after-removal selection '(1 2) 4)
    (check (equal '(0 1 2 3) (orfeus/gui::picker-selection-list selection))
           "Removal did not renumber the rest: ~S" (orfeus/gui::picker-selection-list selection))
    (check (= 1 (orfeus/gui::picker-selection-cursor selection))
           "The cursor did not land where the first removed frame was"))
  ;; The stash keeps each frame once and drops by index.
  (let* ((a (orfeus/gui::make-picker-entry "a.orf" #P"/card/a.orf"))
         (b (orfeus/gui::make-picker-entry "b.orf" #P"/card/b.orf"))
         (c (orfeus/gui::make-picker-entry "c.orf" #P"/card/c.orf")))
    (multiple-value-bind (stash added) (orfeus/gui::picker-stash-add #() (list a b))
      (check (= 2 added) "Two new frames were not counted as two")
      (multiple-value-bind (stash added) (orfeus/gui::picker-stash-add stash (list b c))
        (check (= 1 added) "A frame already stashed was counted again")
        (check (equal '("a.orf" "b.orf" "c.orf")
                      (map 'list #'orfeus/gui::picker-entry-name stash))
               "The stash lost its order")
        (check (equal '("a.orf" "c.orf")
                      (map 'list #'orfeus/gui::picker-entry-name
                           (orfeus/gui::picker-stash-remove stash '(1))))
               "Removing by index took the wrong frame"))))
  ;; Geometry: cells, bands, scrolling to show, type-ahead.
  (let ((width orfeus/gui::*picker-cell-width*)
        (height orfeus/gui::*picker-cell-height*))
    (check (= 3 (orfeus/gui::picker-grid-columns (+ 8 (* 3 width))))
           "Three cells did not fit where three fit")
    (check (eql 4 (orfeus/gui::picker-cell-at (+ 4 width 10) (+ height 10) 0 3 9))
           "The cell under a point was misread")
    (check (null (orfeus/gui::picker-cell-at (+ 4 width 10) (+ height 10) 0 3 4))
           "A point past the last cell found one")
    (check (equal '(0 1 3 4)
                  (orfeus/gui::picker-cells-in-rectangle 10 10 (+ 4 width 20) (+ height 20) 0 3 9))
           "The band did not pick the cells it crossed")
    ;; Cell 3 is the second row; a view ten pixels short of two rows scrolls
    ;; by exactly those ten pixels, no further.
    (check (= 10 (orfeus/gui::picker-scroll-to-show 0 3 3 (- (* 2 height) 10)))
           "Scrolling to a cell below the view did not bring it in by the least")
    (check (= 0 (orfeus/gui::picker-scroll-to-show 0 1 3 (* 2 height)))
           "Scrolling moved for a cell already in view"))
  (check (eql 2 (orfeus/gui::picker-type-ahead-match #("Alpha" "beta" "Bravo" "gamma") "br" 0))
         "Type-ahead did not find the name")
  (check (eql 0 (orfeus/gui::picker-type-ahead-match #("Alpha" "beta") "a" 1))
         "Type-ahead did not wrap round")
  ;; Settings are kept and validated, recent folders capped.
  (let ((pathname (format nil "/tmp/orfeus-picker-settings-~D.sexp" (get-universal-time))))
    (check (orfeus/gui::picker-write-settings
            (list :directory "/cards/" :filter :pictures :sort :newest
                  :recent (loop for index below 12 collect (format nil "/r~D/" index))
                  :junk 42)
            pathname)
           "Settings did not write")
    (let ((read (orfeus/gui::picker-read-settings pathname)))
      (check (equal "/cards/" (getf read :directory)) "The folder did not come back")
      (check (eq :pictures (getf read :filter)) "The filter did not come back")
      (check (= orfeus/gui::*picker-recent-limit* (length (getf read :recent)))
             "Recent folders were not capped")
      (check (null (getf read :junk)) "An unknown setting came back"))
    (delete-file pathname))
  (check (equal '("/b/" "/a/" "/c/")
                (orfeus/gui::picker-remember-directory '("/a/" "/b/" "/c/") "/b/"))
         "A revisited folder did not move to the front")
  (check (equal "DCIM/100OMSYS"
                (orfeus/gui::picker-directory-label #P"/run/media/card/DCIM/100OMSYS/"))
         "A card folder was not labelled by its parent too")
  (check (string= "1.1 MB" (orfeus/gui::picker-size-text 1133082))
         "A megabyte did not read as one"))
